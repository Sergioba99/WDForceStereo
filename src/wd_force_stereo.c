// WDForceStereo v1.2 - Watch Dogs (2014), configurable XAudio2 2.7 stereo
// downmix x64, no CRT. Drop as dinput8.dll next to watch_dogs.exe.
//
...
//   - Forcing only the mastering voice to 2ch breaks that 6->6 matrix and loses
//   dialogue.
//   - v0.7 confirms dialogue-like bursts live in the source Front Center
//   channel.
//
// v1.2 targets the actual 6->2 matrix observed after forcing the mastering
// voice to stereo:
...
//   3) FL/FR, FC, LFE and rear channels can be tuned independently, plus
//   MasterGain.
//
// Defaults intentionally reproduce v1.1 working behavior:
//   FrontGain=1.0, CenterGain=1.0, SurroundGain=0.0, LFEGain=0.0,
//   MasterGain=1.0.
//
#ifdef __cplusplus
extern "C" {
...
typedef struct _XAUDIO2_SEND_DESCRIPTOR_MIN {
  UINT Flags;
  void *pOutputVoice;
} XAUDIO2_SEND_DESCRIPTOR_MIN;
typedef struct _XAUDIO2_VOICE_SENDS_MIN {
  UINT SendCount;
  XAUDIO2_SEND_DESCRIPTOR_MIN *pSends;
} XAUDIO2_VOICE_SENDS_MIN;
...
typedef HRESULT(WINAPI *PFN_SetOutputVoices27)(void *,
...
typedef struct _VOICE_INFO {
  ...
  WORD cbSize;
  DWORD extMask;
  DWORD subFormatData1;
  UINT explicitSendCount;
  void *explicitDest;
} VOICE_INFO;
typedef struct _VTBL_HOOK {
  void **vtbl;
  PFN_SetOutputVoices27 setVoices;
...
static void BuildPaths(void) {
  WCHAR exe[MAX_PATH];
  DWORD n = GetModuleFileNameW(NULL, exe, MAX_PATH), i, cut = 0;
  if (!n || n >= MAX_PATH)
    return;
  for (i = 0; i < n; ++i) {
    g_gameDir[i] = exe[i];
  ...
  for (i = 0; i < cut && i < MAX_PATH - 1; ++i) {
    g_logPath[i] = g_gameDir[i];
    g_iniPath[i] = g_gameDir[i];
  }
  {
    const WCHAR logName[] = L"WDForceStereo.log";
    UINT j = 0;
  ...
  {
    const WCHAR iniName[] = L"WDForceStereo.ini";
    UINT j = 0;
...
typedef DWORD(WINAPI *PFN_GetPrivateProfileStringW)(LPCWSTR, LPCWSTR, LPCWSTR,
                                                    WCHAR *, DWORD, LPCWSTR);
...
static float ReadIniGain(PFN_GetPrivateProfileStringW gp, LPCWSTR key,
                         LPCWSTR def, float fallback) {
  WCHAR b[64];
  float v = fallback;
  b[0] = 0;
  gp(L"Audio", key, def, b, 64, g_iniPath);
  if (ParseFloatW(b, &v)) {
    if (v < 0.0f)
      v = 0.0f;
    if (v > 4.0f)
...
static void LoadConfig(void) {
  HMODULE k;
  PFN_GetPrivateProfileStringW gp;
  WCHAR b[64];
  if (!g_iniPath[0])
    BuildPaths();
  ...
  b[0] = 0;
  gp(L"Debug", L"Log", L"1", b, 64, g_iniPath);
...
static ENGINE_INFO *FindEngineInfo(void *e, int create) {
  UINT i;
  for (i = 0; i < g_engineCount; ++i)
    if (g_engines[i].engine == e)
      return &g_engines[i];
  if (create && g_engineCount < MAX_ENGINES) {
    ENGINE_INFO *x = &g_engines[g_engineCount++];
    memset(x, 0, sizeof(*x));
    x->engine = e;
    return x;
  }
...
static VOICE_INFO *FindVoiceInfo(void *v, int create) {
  UINT i;
  for (i = 0; i < g_voiceCount; ++i)
    if (g_voices[i].voice == v)
      return &g_voices[i];
  if (create && g_voiceCount < MAX_VOICES) {
    VOICE_INFO *x = &g_voices[g_voiceCount++];
    memset(x, 0, sizeof(*x));
    x->voice = v;
    return x;
  }
...
static UINT ReadDword(const BYTE *p) {
  UINT v;
  memcpy(&v, p, 4);
  return v;
}

static void ParseFormat(VOICE_INFO *vi, const WAVEFORMATEX_MIN *f) {
  const BYTE *b = (const BYTE *)f;
  if (!vi || !f)
    return;
  vi->channels = f->nChannels;
  vi->rate = f->nSamplesPerSec;
  vi->tag = f->wFormatTag;
  vi->bits = f->wBitsPerSample;
  vi->blockAlign = f->nBlockAlign;
  vi->cbSize = f->cbSize;
  vi->extMask = 0;
  vi->subFormatData1 = 0;
  if (f->wFormatTag == WAVE_FORMAT_EXTENSIBLE && f->cbSize >= 22) {
    vi->extMask = ReadDword(b + 20);
    vi->subFormatData1 = ReadDword(b + 24);
  }
}
static void LogSourceVoiceCreation(const VOICE_INFO *vi,
                                   const XAUDIO2_VOICE_SENDS_MIN *sends,
                                   const void *effects) {
  CHAR l[720];
  UINT p = 0;
  p = AppendString(l, p, sizeof(l), "SourceVoice created: ptr=");
  p = AppendPointer(l, p, sizeof(l), vi->voice);
  p = AppendString(l, p, sizeof(l), " engine=");
  p = AppendPointer(l, p, sizeof(l), vi->engine);
  p = AppendString(l, p, sizeof(l), " ch=");
  p = AppendUInt(l, p, sizeof(l), vi->channels);
  p = AppendString(l, p, sizeof(l), " rate=");
  p = AppendUInt(l, p, sizeof(l), vi->rate);
  p = AppendString(l, p, sizeof(l), " tag=");
  p = AppendHex32(l, p, sizeof(l), (DWORD)vi->tag);
  p = AppendString(l, p, sizeof(l), " bits=");
  p = AppendUInt(l, p, sizeof(l), vi->bits);
  p = AppendString(l, p, sizeof(l), " align=");
  p = AppendUInt(l, p, sizeof(l), vi->blockAlign);
  p = AppendString(l, p, sizeof(l), " cbSize=");
  p = AppendUInt(l, p, sizeof(l), vi->cbSize);
  if (vi->tag == WAVE_FORMAT_EXTENSIBLE) {
    p = AppendString(l, p, sizeof(l), " extMask=");
    p = AppendHex32(l, p, sizeof(l), vi->extMask);
    p = AppendString(l, p, sizeof(l), " subFmtData1=");
    p = AppendHex32(l, p, sizeof(l), vi->subFormatData1);
  }
  p = AppendString(l, p, sizeof(l), " effects=");
  p = AppendPointer(l, p, sizeof(l), effects);
  if (!sends)
    p = AppendString(l, p, sizeof(l), " sends=DEFAULT_MASTERING");
  else {
    p = AppendString(l, p, sizeof(l), " sends=");
    p = AppendUInt(l, p, sizeof(l), sends->SendCount);
    if (sends->SendCount && sends->pSends) {
      p = AppendString(l, p, sizeof(l), " firstDest=");
      p = AppendPointer(l, p, sizeof(l), sends->pSends[0].pOutputVoice);
    }
  }
  LogLine(l);
}

static UINT ResolveDestinationChannels(VOICE_INFO *vi,
                                       const XAUDIO2_VOICE_SENDS_MIN *sends,
                                       void **destOut) {
  XAUDIO2_VOICE_DETAILS_MIN d;
  ENGINE_INFO *ei;
  if (destOut)
    *destOut = NULL;
  if (sends) {
    if (sends->SendCount != 1 || !sends->pSends)
      return 0;
    if (destOut)
      *destOut = sends->pSends[0].pOutputVoice;
    if (!sends->pSends[0].pOutputVoice)
      return 0;
    memset(&d, 0, sizeof(d));
    {
      void **vt = *(void ***)sends->pSends[0].pOutputVoice;
      if (!vt || !vt[0])
        return 0;
      ((PFN_GetVoiceDetails27)vt[0])(sends->pSends[0].pOutputVoice, &d);
    }
    return d.InputChannels;
  }
  ei = FindEngineInfo(vi->engine, 0);
  if (ei) {
    if (destOut)
      *destOut = ei->mastering;
    return ei->masteringChannels;
  }
  return 0;
}
...
static void Compose6x6To6x2(const float *game6, float *out12) {
  ...
  for (d = 0; d < 2; ++d) {
    for (s = 0; s < 6; ++s) {
      float sum = 0.0f;
      for (v = 0; v < 6; ++v)
        sum += k[v + 6 * d] * game6[s + 6 * v];
      out12[s + 6 * d] = sum * g_masterGain;
    }
  }
}
static HRESULT WINAPI
Hook_SetOutputVoices(void *This, const XAUDIO2_VOICE_SENDS_MIN *sends) {
  VTBL_HOOK *h = FindVtableHook(This);
  VOICE_INFO *vi = FindVoiceInfo(This, 0);
  HRESULT hr;
  if (!h || !h->setVoices)
    return E_FAIL;
  ...
  if (vi && vi->channels == 6) {
    ...
    UINT p = 0;
    void *dest = NULL;
    UINT dc = ResolveDestinationChannels(vi, sends, &dest);
    p = AppendString(l, p, sizeof(l), "SetOutputVoices on 6ch source: hr=");
    p = AppendHex32(l, p, sizeof(l), (DWORD)hr);
    p = AppendString(l, p, sizeof(l), " resolvedDestCh=");
...
static HRESULT WINAPI Hook_SetOutputMatrix(void *This, void *dest, UINT sc,
  ...
  if (vi && vi->channels == 6) {
    p = AppendString(l, p, sizeof(l), "Game SetOutputMatrix on 6ch source: ");
    ...
    if (m && sc == 6 && dc == 2) {
      ...
      p = 0;
      p = AppendString(l, p, sizeof(l), "  L row [FL FR FC LFE BL BR] = ");
      p = AppendFloat3(l, p, sizeof(l), m[0]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[1]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[2]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[3]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[4]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[5]);
      LogLine(l);
      p = 0;
      p = AppendString(l, p, sizeof(l), "  R row [FL FR FC LFE BL BR] = ");
      p = AppendFloat3(l, p, sizeof(l), m[6]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[7]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[8]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[9]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[10]);
      p = AppendString(l, p, sizeof(l), " ");
      p = AppendFloat3(l, p, sizeof(l), m[11]);
      LogLine(l);

#ifndef WD_DIAGNOSTIC_6TO2
      {
        ENGINE_INFO *ei = FindEngineInfo(vi->engine, 0);
        if (ei && ei->mastering == dest && ei->masteringChannels == 2) {
          ...
          p = 0;
          p = AppendString(l, p, sizeof(l), "  -> CONFIGURED DOWNMIX 6->2 hr=");
          p = AppendHex32(l, p, sizeof(l), (DWORD)hr);
          LogLine(l);
          p = 0;
          p = AppendString(l, p, sizeof(l),
                           "     L final [FL FR FC LFE BL BR] = ");
          p = AppendFloat3(l, p, sizeof(l), t[0]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[1]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[2]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[3]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[4]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[5]);
          LogLine(l);
          p = 0;
          p = AppendString(l, p, sizeof(l),
                           "     R final [FL FR FC LFE BL BR] = ");
          p = AppendFloat3(l, p, sizeof(l), t[6]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[7]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[8]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[9]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[10]);
          p = AppendString(l, p, sizeof(l), " ");
          p = AppendFloat3(l, p, sizeof(l), t[11]);
          LogLine(l);
          return hr;
        }
...
static void HookSourceVtable(void *voice) {
  ...
  if (!VirtualProtect(&vt[1], sizeof(void *), PAGE_EXECUTE_READWRITE, &old)) {
    LogLine("ERROR: cannot hook SetOutputVoices.");
    return;
  }
  g_vtbls[g_vtblCount].vtbl = vt;
  g_vtbls[g_vtblCount].setVoices = (PFN_SetOutputVoices27)vt[1];
  vt[1] = (void *)&Hook_SetOutputVoices;
  {
    DWORD x;
    VirtualProtect(&vt[1], sizeof(void *), old, &x);
  }
  ...
  {
    DWORD x;
    VirtualProtect(&vt[16], sizeof(void *), old, &x);
  }
  ++g_vtblCount;
  LogLine("Installed source vtable hooks: SetOutputVoices + SetOutputMatrix.");
}

static HRESULT WINAPI Hook_CreateSourceVoice27(void *This, void **pp,
                                               const WAVEFORMATEX_MIN *f,
                                               UINT flags, float ratio,
                                               void *cb, const void *sendList,
                                               const void *fx) {
  HRESULT hr =
      g_realCreateSourceVoice(This, pp, f, flags, ratio, cb, sendList, fx);
  if (!FAILED(hr) && pp && *pp) {
    VOICE_INFO *vi = FindVoiceInfo(*pp, 1);
    if (vi) {
      ...
      ParseFormat(vi, f);
      if (sendList) {
        const XAUDIO2_VOICE_SENDS_MIN *s =
            (const XAUDIO2_VOICE_SENDS_MIN *)sendList;
        vi->explicitSendCount = s->SendCount;
        if (s->SendCount && s->pSends)
          vi->explicitDest = s->pSends[0].pOutputVoice;
      }
      LogSourceVoiceCreation(vi, (const XAUDIO2_VOICE_SENDS_MIN *)sendList, fx);
      HookSourceVtable(*pp);
    }
  }
  return hr;
...
static HRESULT WINAPI Hook_CreateMasteringVoice27(void *This, void **pp,
  ...
  HRESULT hr =
      g_realCreateMasteringVoice(This, pp, passedCh, rate, flags, dev, fx);
  CHAR l[420];
  UINT p = 0;
  ...
  if (!FAILED(hr) && pp && *pp) {
    XAUDIO2_VOICE_DETAILS_MIN d;
    ENGINE_INFO *ei = FindEngineInfo(This, 1);
    memset(&d, 0, sizeof(d));
    {
      void **vt = *(void ***)*pp;
      if (vt && vt[0])
        ((PFN_GetVoiceDetails27)vt[0])(*pp, &d);
    }
    if (ei) {
      ei->mastering = *pp;
    ...
    p = AppendUInt(l, p, sizeof(l), d.InputChannels);
    p = AppendString(l, p, sizeof(l), " actualRate=");
    p = AppendUInt(l, p, sizeof(l), d.InputSampleRate);
    if (ch == 6)
...
static void InstallEngineHooks(void *xa) {
  ...
  {
    DWORD x;
    VirtualProtect(&vt[8], 3 * sizeof(void *), old, &x);
  }
  g_hooksInstalled = 1;
  LogLine("XAudio2 2.7 hooks installed: 6ch mastering -> 2ch + configurable "
          "6->2 downmix.");
}
static void InitCoCreate(void) {
  HMODULE o;
  if (g_realCoCreateInstance)
    return;
  o = GetModuleHandleW(L"ole32.dll");
  if (!o)
    o = LoadLibraryW(L"ole32.dll");
  if (o)
    g_realCoCreateInstance =
        (PFN_CoCreateInstance)GetProcAddress(o, "CoCreateInstance");
}
static int Probe(void) {
  HMODULE o, x;
  PFN_CoInitializeEx ci;
  PFN_CoUninitialize cu;
  HRESULT ih, hr;
  void *xa = NULL;
  int un = 0;
  x = LoadLibraryW(L"XAudio2_7.dll");
  if (!x) {
  ...
  if (!o)
    return 0;
  ci = (PFN_CoInitializeEx)GetProcAddress(o, "CoInitializeEx");
  cu = (PFN_CoUninitialize)GetProcAddress(o, "CoUninitialize");
  InitCoCreate();
  if (!g_realCoCreateInstance)
    return 0;
  ih = ci ? ci(NULL, 0) : S_OK;
  if (ih == S_OK || ih == 1)
    un = 1;
  hr = g_realCoCreateInstance(&kCLSID_XAudio2, NULL, CLSCTX_INPROC_SERVER,
                              &kIID_IXAudio2, &xa);
  {
    CHAR l[150];
    UINT p = 0;
...
static DWORD WINAPI HookThread(LPVOID u) {
  ...
  LogLine(
      "Matrix order: destination rows L/R; source columns FL FR FC LFE BL BR.");
  if (Probe())
...
BOOL WINAPI DllMain(HINSTANCE h, DWORD r, LPVOID x) {
  ...
  if (r == DLL_PROCESS_ATTACH) {
    ...
    BuildPaths();
    t = CreateThread(NULL, 0, HookThread, NULL, 0, NULL);
    if (t)
      CloseHandle(t);
  }
  return TRUE;
